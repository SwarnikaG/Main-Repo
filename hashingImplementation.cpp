...
class hashMap{
    ...
    public:
        ...
        void insert(string key, int value){
            ...
            int flag=0;
            while(temp){ //see in main function how apple is used twice but while printing it just prints once because we are skipping it
                if(temp->key==key){
                    flag=1;
                    break;
                }
            ...
            if(flag!=1){ //this flag is to avoid insertion of same keys
            ...
            float loadFactor = cs/(ms*1.0); //multiplying with 1.0 to convert the integer to float
            if(loadFactor>0.7){
                rehash();
            }
        }
        ...
        node* searchKey(string key){ //can make this function as return type bool and return true when key is found
            int index=hashfn(key);
            if(bucket[index]){
